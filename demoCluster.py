...
if len(cluster_target) > 0:
	pl.subplot(411)
	pl.plot(range(cluster_size[0]), model.denormalize_target(cluster_prediction[0], target_val[1][0], target_val[1][1]), '-',range(cluster_size[0]) , model.denormalize_target(cluster_target[0], target_val[1][0], target_val[1][1]), '.')
	pl.legend(['prediction value', 'actual value'])
	pl.title('cluster 0')

if len(cluster_target) > 1:
	pl.subplot(412)
	pl.plot(range(cluster_size[1]), model.denormalize_target(cluster_prediction[1], target_val[1][0], target_val[1][1]), '-',range(cluster_size[1]) , model.denormalize_target(cluster_target[1], target_val[1][0], target_val[1][1]), '.')
	pl.legend(['prediction value', 'actual value'])
	pl.title('cluster 1')
